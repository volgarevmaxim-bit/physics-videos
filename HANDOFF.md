# physics-videos — Handoff после этапа 4 (v0.2.0) — ✅ ОПУБЛИКОВАН

> Локальная копия: `C:\Users\volga\physics-videos` (main = origin/main, запушено 2026-09-22).
> Теги: `v0.1.0`, `v0.2.0` — на GitHub. Сайт: https://volgarevmaxim-bit.github.io/physics-videos/ — задеплоен и верифицирован (все маршруты 200, E2E в браузере: гейт → «Сын» → сиды в localStorage).
> Issues бэклога: №2–№6 (метки не присвоены — токен без прав на редактирование; probe-№1 закрыть вручную).
...
## Ручные хвосты (мелочь, через web UI GitHub)

1. Закрыть probe-issue №1 (токен не умеет закрывать).
2. Присвоить метки issues №2–№6: №2 content; №3 ui; №4 ui; №5 automation; №6 content+automation (метки в репо уже есть).
3. В браузере сына: проверить, что 3 видео недели не предлагаются (сид сработает при первом заходе).
