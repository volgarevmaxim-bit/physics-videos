# physics-videos — Handoff после этапа 4 (v0.2.2) — ✅ ОПУБЛИКОВАН

> Локальная копия: `C:\Users\volga\physics-videos` (main = origin/main, запушено 2026-09-22).
> Теги: `v0.1.0`–`v0.2.2` — на GitHub. Сайт: https://volgarevmaxim-bit.github.io/physics-videos/ — v0.2.2 live (CDN кэш app.js до 10 минут).
> Issues бэклога: №2–№6 с метками content/ui/automation; probe-№1 закрыт.
...
1. В браузере сына: проверить, что 3 видео недели не предлагаются (сид сработает при первом заходе).

## После v0.2.1/v0.2.2

- v0.2.1: utility-bar над видео + гамбургер (смена профиля); 3 просмотренных видео заменены на топ-3 ранжира GetAClass (Электрические заряды, Самозапускающийся сифон, Наклонная плоскость; rutube-id через открытый API `rutube.ru/api/search/video/?query=`).
- v0.2.2: подписи видео в списке ≤2 слов (стоп-слова не занимают слот).
- Pages-автотриггер на пуш иногда молчит: лечится пустым коммитом. CDN кэширует app.js 10 минут.
